...
- **aprobada** — boolean flag; articles default to false on creation via `agregarNoticiaSinEdicion`, true via `agregarNoticia`. Only `@RolesAllowed` admins can approve via `NoticiasService.aprobarNoticia`.
...
- Only ADMIN_ROOT or ADMIN_DOTA can delete or approve articles; enforced by `@RolesAllowed` on `eliminarNoticia` and `@RolesAllowed` on `aprobarNoticia` in `NoticiasService`.
